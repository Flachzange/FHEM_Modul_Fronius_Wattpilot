...
- idle-refresh, inbound-liveness recovery, and shared-clock telemetry publication.
...
An established session for the empirically evidenced `wattpilot_flex` device profile also owns one typed `inbound_watchdog` timer. Every completely decoded JSON document updates `lastInboundJsonAt`; incomplete and invalid input does not. The watchdog checks every 30 seconds and replaces a session after at least 180 seconds without a complete document. Recovery uses the same lifecycle-generation invalidation and guarded reconnect path, so stale callbacks cannot close a newer session. The watchdog is independent of telemetry publication, `interval`, and `update_while_idle`. The legacy `devicetype=wattpilot` profile is deliberately not assigned this timeout because no sufficiently bounded idle-message cadence is evidenced for it. Reload initializes the two persistent connection-diagnostic readings without events and installs or replaces exactly one watchdog only when the retained Flex device is already connected and DevIo-open. The boolean `inboundWatchdog` attribute has effective default `1`; value `0` cancels only this typed timer. Because FHEM invokes `AttrFn` before storing or deleting the attribute, the callback passes the effective new value explicitly: disabling removes the timer immediately, while enabling or deleting the override re-arms one eligible live Flex timer and resets `lastInboundJsonAt` to grant a full inactivity window. Reload respects a stored `0`. Ordinary socket, lifecycle, idle-refresh, and manual reconnect ownership is deliberately unaffected.
...
| `lastInboundJsonAt` | current watched `wattpilot_flex` session; refreshed by every complete decoded JSON document and removed by session invalidation |
| `connectionLastReconnectReason`, `connectionAutomaticReconnectCount` | persistent public lifecycle diagnostics; not cleared with session-scoped helper state |

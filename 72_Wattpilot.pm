...
my @WATTPILOT_READING_DEFINITION = (
    ...
    [qw(load_balancing_grid_connection_current configLoadBalancingGridConnectionCurrent configuration
            status:lot.amp immediate none lot_amp integer nonnegative_integer none)],
    [qw(load_balancing_supply_line_current configLoadBalancingSupplyLineCurrent configuration
            status:lot.sta immediate none lot_sta integer nonnegative_integer none)],
    ...
    [qw(device_controller_firmware_version deviceControllerFirmwareVersion device_health
            status:cc4.firmware_version interval none device_health text string none)],
    [qw(device_controller_firmware_crc deviceControllerFirmwareCRC device_health
            status:cc4.firmware_crc interval none device_health text string none)],
    [qw(device_controller_firmware_integrity deviceControllerFirmwareIntegrity device_health
            status:cc4.firmware_integrity interval none device_health text string none)],
    [qw(device_controller_stack_size deviceControllerStackSize device_health
            status:cc4.stack_size interval none device_health integer nonnegative_integer none)],
    [qw(device_controller_reset_reason deviceControllerResetReason device_health
            status:cc4.reset_reason interval none device_health text string none)],
    [qw(device_controller_mid_firmware_version deviceControllerMidFirmwareVersion device_health
            status:cc4.mid_firmware_version interval none device_health text string none)],
    [qw(device_controller_hardware_id deviceControllerHardwareId device_health
            status:cc4.hwid interval none device_health text string none)],
    [qw(diag_temperature_sensor_1 diag_temperatureSensor1 optional_diagnostic
            status:tma[0] interval diagnostic diagnostic decimal2 number none)],
    [qw(diag_temperature_sensor_2 diag_temperatureSensor2 optional_diagnostic
            status:tma[1] interval diagnostic diagnostic decimal2 number none)],
    [qw(diag_temperature_sensor_3 diag_temperatureGridConnector optional_diagnostic
            status:tma[2] interval diagnostic diagnostic decimal2 number none)],
    [qw(diag_temperature_sensor_4 diag_temperatureCurrentSensor optional_diagnostic
            status:tma[3] interval diagnostic diagnostic decimal2 number none)],
    [qw(diag_temperature_sensor_5 diag_temperatureType2ScrewTerminals optional_diagnostic
            status:tma[4] interval diagnostic diagnostic decimal2 number none)],
    [qw(diag_temperature_sensor_6 diag_temperatureMID optional_diagnostic
            status:tma[5] interval diagnostic diagnostic decimal2 number none)],
    [qw(diag_fbuf_akku_mode diag_fbuf_akkuMode optional_diagnostic
            status:fbuf_akkuMode interval diagnostic diagnostic diagnostic2 raw_scalar none)],
    ...
    [qw(diag_pvopt_phase_wish_mode diag_pvopt_phaseWishMode optional_diagnostic
            status:pwm interval diagnostic diagnostic enum integer phase_wish)],
...
for my $definition (@WATTPILOT_READING_DEFINITION) {
    ...
    next if $policy{source} !~ /^status:([A-Za-z0-9_]+(?:\.[A-Za-z0-9_]+|\[\d+\])?)$/;
    my $protocol_key = $1;
    $protocol_key = $1
        if $policy{validator} eq 'nrg'
        && $protocol_key =~ /^([A-Za-z0-9_]+)\[\d+\]$/;
...
for my $definition (@WATTPILOT_READING_DEFINITION) {
    ...
    if ($policy->{source} =~ /^status:([A-Za-z0-9_]+(?:\.[A-Za-z0-9_]+|\[\d+\])?)$/
        && $policy->{validator} ne 'nrg') {
...
my @WATTPILOT_COMMAND_DEFINITION = (
    ...
    ['pv_battery_discharge', 'pvBatteryDischarge', 'widgetList,3,select,off,on,3,textField,SoC%,4', 'none', 'special', '<off|0> [<0-100>] | <on|1> <0-100>', 'usage'],
...
my %WATTPILOT_LOAD_BALANCING_PHASE_ASSIGNMENT = (
    '1,0,0' => [1],
    '0,1,0' => [2],
    '0,0,1' => [3],
    '1,2,3' => [1, 2, 3],
...
sub Wattpilot_NormalizeStatusValue($$) {
    ...
    if ($kind eq 'string' || $kind eq 'nonempty_string') {
        return undef if !Wattpilot_IsJsonString($value);
        return undef if $kind eq 'nonempty_string' && $value eq '';
        return $value;
    }
    ...
    if ($kind eq 'phase_assignment') {
        return undef if ref($value) ne 'ARRAY' || @$value != 3;
        return undef if grep { !Wattpilot_IsJsonInteger($_) } @$value;

        my $signature = join(',', map { int($_) } @$value);
        return $WATTPILOT_LOAD_BALANCING_PHASE_ASSIGNMENT{$signature};
    }
...
sub Wattpilot_ExtractStatusSource($$) {
    my ($status, $source) = @_;
    return (0, undef, 1) if ref($status) ne 'HASH';

    if ($source =~ /^([A-Za-z0-9_]+)$/) {
        return (0, undef, 1) if !exists $status->{$1};
        return (1, $status->{$1}, 1);
    }
    if ($source =~ /^([A-Za-z0-9_]+)\[(\d+)\]$/) {
        my ($root, $index) = ($1, $2);
        return (0, undef, 1) if !exists $status->{$root};
        return (1, undef, 1) if !defined $status->{$root};
        return (1, undef, 0) if ref($status->{$root}) ne 'ARRAY';
        return (0, undef, 1) if $index >= @{$status->{$root}};
        return (1, $status->{$root}[$index], 1);
    }
    if ($source =~ /^([A-Za-z0-9_]+)\.([A-Za-z0-9_]+)$/) {
        my ($root, $child) = ($1, $2);
        return (0, undef, 1) if !exists $status->{$root};
        return (1, undef, 1) if !defined $status->{$root};
        return (1, undef, 0) if ref($status->{$root}) ne 'HASH';
        return (0, undef, 1) if !exists $status->{$root}{$child};
        return (1, $status->{$root}{$child}, 1);
    }
    die "Unsupported Wattpilot status source: $source";
}
...
sub Wattpilot_NormalizeStatus($$) {
    ...
    for my $key (keys %WATTPILOT_STATUS_SCHEMA) {
        my ($exists, $value, $shape_valid) =
            Wattpilot_ExtractStatusSource($input_status, $key);
        next if !$exists;
        ...
        my $normalized = Wattpilot_NormalizeStatusValue(
            $value, $WATTPILOT_STATUS_SCHEMA{$key});
...
sub Wattpilot_SetPvBatteryDischarge($@) {
    ...
    return Wattpilot_PvBatteryDischargeUsage($name)
        if !defined($enabled) || !exists($enabled_value{$enabled});
...
<ul>
  ...
  <p>Version 2.1.18 lets <code>pvBatteryDischarge off</code> or <code>0</code> omit the SoC threshold. That form sends only <code>pdte=false</code> as a JSON boolean and leaves <code>pdt</code> unchanged. Enabling with <code>on</code> or <code>1</code> still requires a whole SoC from <code>0</code> through <code>100</code>. Existing two-value disable forms and FHEMWEB comma input remain compatible; an empty SoC field is accepted only for <code>off</code>.</p>
  <table class="block wide">
    ...
    <tr><td><code>diag_temperatureSensor4</code></td><td><code>diag_temperatureCurrentSensor</code></td></tr>
    <tr><td><code>diag_temperatureSensor5</code></td><td><code>diag_temperatureType2ScrewTerminals</code></td></tr>
    <tr><td><code>diag_temperatureSensor6</code></td><td><code>diag_temperatureMID</code></td></tr>
  </table>
  <table class="block wide">
  ...
  <ul>
    <li><code>set &lt;name&gt; password &lt;secret&gt;</code><br>
        Stores or replaces the password under stable FUUID-based keys and starts a controlled reconnect when runtime state permits. Replacing it invalidates the stored derived hash and rolls back the previous credentials if persistence fails. Rename does not rewrite credentials. Deleting the FHEM device removes the stored password and derived hash together and attempts rollback if that deletion fails. Storage errors remain distinguishable from a missing password.</li>
    ...
    <li><code>set &lt;name&gt; pvBatteryDischarge &lt;off|0&gt; [&lt;0-100&gt;] | &lt;on|1&gt; &lt;0-100&gt;</code><br>
        Combines <code>dischargeEnabled</code> and <code>dischargeUntilSoC</code> with conditional arity. <code>off</code> or <code>0</code> without a threshold sends only <code>pdte=false</code> and preserves the current <code>pdt</code> value. If a threshold is supplied while disabling, <code>pdte=false</code> is confirmed before the new <code>pdt</code> value is sent. <code>on</code> or <code>1</code> always requires a whole threshold from <code>0</code> through <code>100</code>; <code>pdt</code> is confirmed before <code>pdte=true</code>. FHEMWEB keeps the <code>off|on</code> selector and compact <code>SoC%</code> field; an empty field is accepted only for <code>off</code>. Related grouped writes are blocked while a request is pending, and readings remain device-confirmed only.</li>
  ...
  <ul>
    ...
    <li><code>diagnosticReadings &lt;0|1&gt;</code><br>
        <code>0</code> is the default and immediately deletes all optional <code>diag_...</code> readings and clears their cache/dirty state; deleting the attribute has the same effect. <code>1</code> enables the twenty-two optional diagnostic readings on the normal <code>interval</code>, eligible while charging or with <code>update_while_idle=1</code>. The fifteen raw scalar diagnostics format JSON numbers with exactly two decimal places, preserve strings, and expose JSON booleans as <code>0|1</code>. <code>diag_pvopt_phaseWishMode</code> uses its documented compatibility enum with an explicit <code>unknown:&lt;value&gt;</code> fallback. Missing, <code>null</code>, object, array, or invalid values preserve the previous reading.</li>
    ...
    <li><code>rawJSONLog &lt;0|1&gt;</code><br>
        <code>0</code> is the default. Exact JSON is logged only with both <code>rawJSONLog=1</code> and <code>verbose=5</code>. This can expose sensitive authentication, network, device, and operational data. DevIo/HttpUtils core logs may still contain endpoint details at high verbosity.</li>
    <li><code>authHash &lt;auto|pbkdf2|bcrypt&gt;</code><br>
        <code>auto</code> is the default. It accepts announced PBKDF2 or bcrypt; a missing hash selects PBKDF2 only for the evidenced predecessor <code>devicetype=wattpilot</code>, protocol-2 profile. Setting, changing, or deleting the attribute invalidates the stored derived hash and current session and schedules one fresh login when possible.</li>
    <li><code>authHashCost &lt;4-14&gt;</code><br>
        Integer bcrypt cost for newly derived authentication hashes; the default is <code>8</code>. It affects bcrypt only. Setting, changing, or deleting the attribute invalidates the stored derived hash and current session and schedules one fresh login when possible.</li>
  </ul>
  <br>
  ...
  <ul>
    ...
    <li><code>configLoadBalancingGridConnectionCurrent</code>, <code>configLoadBalancingSupplyLineCurrent</code><br>Non-negative integers from <code>lot.amp</code> and <code>lot.sta</code>. Independent app changes to 5 A and 25 A confirmed the mappings to maximum grid-connection current and maximum supply-line current. <code>lot.dyn</code> is deliberately not exposed.</li>
    <li><code>configLoadBalancingPhaseAssignment</code><br>Fixed three-slot vector from <code>map</code>. The confirmed one-phase vectors <code>[1,0,0]</code>, <code>[0,1,0]</code>, and <code>[0,0,1]</code> render as <code>L1</code>, <code>L2</code>, and <code>L3</code>. Every permutation of <code>1,2,3</code> is accepted and rendered in configured order, for example <code>[2,3,1]</code> as <code>L2 L3 L1</code>. Two-phase, malformed, and otherwise unconfirmed vectors preserve the previous reading.</li>
    ...
    <li><code>deviceControllerFirmwareVersion</code>, <code>deviceControllerFirmwareCRC</code>, <code>deviceControllerFirmwareIntegrity</code>, <code>deviceControllerResetReason</code>, <code>deviceControllerMidFirmwareVersion</code>, <code>deviceControllerHardwareId</code><br>Raw strings from the corresponding members of the observed <code>cc4</code> object. They publish on the shared interval without idle gating. No relationship to the main firmware reading, enum, token decoding, or health verdict is inferred.</li>
    <li><code>deviceControllerStackSize</code><br>Raw non-negative integer from <code>cc4.stack_size</code>, published with the other device-health values. Unit and exact meaning remain unconfirmed.</li>
    ...
    <li><code>diag_pvopt_phaseWishMode</code><br>Optional internal phase-wish value from <code>pwm</code>: <code>force3</code>, <code>wish1</code>, <code>wish3</code>, or <code>unknown:&lt;raw-value&gt;</code>. It is distinct from configured <code>configPhaseSwitchMode</code>; no timer or actual phase transition is inferred.</li>
...
<ul>
  ...
  <p>Version 2.1.18 erlaubt bei <code>pvBatteryDischarge off</code> beziehungsweise <code>0</code> das Weglassen des SoC-Grenzwerts. Diese Form sendet ausschließlich <code>pdte=false</code> als JSON-Boolean und lässt <code>pdt</code> unverändert. Das Aktivieren mit <code>on</code> oder <code>1</code> benötigt weiterhin einen ganzzahligen SoC von <code>0</code> bis <code>100</code>. Bestehende Zwei-Wert-Aufrufe zum Deaktivieren und die FHEMWEB-Kommaform bleiben kompatibel; ein leeres SoC-Feld ist nur bei <code>off</code> zulässig.</p>
  <table class="block wide">
  ...
  <ul>
    <li><code>set &lt;name&gt; password &lt;secret&gt;</code><br>
        Speichert oder ersetzt das Passwort unter stabilen FUUID-basierten Schlüsseln und startet einen kontrollierten Reconnect, wenn der Laufzeitzustand dies zulässt. Beim Ersetzen wird der gespeicherte abgeleitete Hash verworfen; schlägt die Persistenz fehl, werden die bisherigen Zugangsdaten wiederhergestellt. Rename schreibt Zugangsdaten nicht um. Beim Löschen des FHEM-Devices werden Passwort und abgeleiteter Hash gemeinsam entfernt; schlägt dies fehl, wird ein Rollback versucht. Speicherfehler bleiben von einem fehlenden Passwort unterscheidbar.</li>
    ...
    <li><code>set &lt;name&gt; pvBatteryDischarge &lt;off|0&gt; [&lt;0-100&gt;] | &lt;on|1&gt; &lt;0-100&gt;</code><br>
        Kombiniert <code>dischargeEnabled</code> und <code>dischargeUntilSoC</code> mit bedingter Argumentzahl. <code>off</code> oder <code>0</code> ohne Grenzwert sendet ausschließlich <code>pdte=false</code> und lässt den vorhandenen <code>pdt</code>-Wert unverändert. Wird beim Deaktivieren ein Grenzwert angegeben, wird <code>pdte=false</code> bestätigt, bevor der neue <code>pdt</code>-Wert folgt. <code>on</code> oder <code>1</code> benötigt immer einen ganzzahligen Grenzwert von <code>0</code> bis <code>100</code>; <code>pdt</code> wird vor <code>pdte=true</code> bestätigt. FHEMWEB behält den <code>off|on</code>-Selektor und das kompakte Feld <code>SoC%</code>; ein leeres Feld ist nur bei <code>off</code> zulässig. Zugehörige gruppierte Schreibzugriffe sind während eines Requests gesperrt, und Readings bleiben ausschließlich gerätebestätigt.</li>
    <li><code>set &lt;name&gt; reconnect</code><br>
  ...
  <ul>
    ...
    <li><code>diagnosticReadings &lt;0|1&gt;</code><br>
        <code>0</code> ist Standard und löscht sofort alle optionalen <code>diag_...</code>-Readings sowie deren Cache-/Dirty-Zustand; das Löschen des Attributs wirkt genauso. <code>1</code> aktiviert die zweiundzwanzig optionalen Diagnosereadings im normalen <code>interval</code>, zulässig beim Laden oder mit <code>update_while_idle=1</code>. Die fünfzehn Rohskalardiagnosen formatieren JSON-Zahlen mit genau zwei Nachkommastellen, erhalten Strings unverändert und geben JSON-Booleans als <code>0|1</code> aus. <code>diag_pvopt_phaseWishMode</code> verwendet seine dokumentierte Kompatibilitäts-Enum mit explizitem <code>unknown:&lt;Wert&gt;</code>-Fallback. Fehlende, <code>null</code>-, Objekt-, Array- oder ungültige Werte erhalten das bisherige Reading.</li>
    <li><code>inboundWatchdog &lt;0|1&gt;</code><br>
    ...
    <li><code>rawJSONLog &lt;0|1&gt;</code><br>
        <code>0</code> ist Standard. Exaktes JSON wird nur mit <code>rawJSONLog=1</code> und <code>verbose=5</code> protokolliert. Dabei können sensible Authentifizierungs-, Netzwerk-, Geräte- und Betriebsdaten sichtbar werden. DevIo-/HttpUtils-Core-Logs können bei hohem Verbose weiterhin Endpoint-Details enthalten.</li>
    <li><code>authHash &lt;auto|pbkdf2|bcrypt&gt;</code><br>
        <code>auto</code> ist Standard. Es akzeptiert angekündigtes PBKDF2 oder bcrypt; ein fehlender Hash wählt nur beim belegten Vorgängerprofil <code>devicetype=wattpilot</code>, Protokoll 2, PBKDF2. Setzen, Ändern oder Löschen des Attributs verwirft den gespeicherten abgeleiteten Hash und die aktuelle Sitzung und plant nach Möglichkeit genau eine frische Anmeldung.</li>
    <li><code>authHashCost &lt;4-14&gt;</code><br>
        Ganzzahliger bcrypt-Kostenfaktor für neu abgeleitete Authentifizierungs-Hashes; Standard ist <code>8</code>. Er wirkt nur auf bcrypt. Setzen, Ändern oder Löschen des Attributs verwirft den gespeicherten abgeleiteten Hash und die aktuelle Sitzung und plant nach Möglichkeit genau eine frische Anmeldung.</li>
  </ul>
  <br>
  ...
  <ul>
    ...
    <li><code>configLoadBalancingGridConnectionCurrent</code>, <code>configLoadBalancingSupplyLineCurrent</code><br>Nicht negative Ganzzahlen aus <code>lot.amp</code> und <code>lot.sta</code>. Unabhängige App-Änderungen auf 5 A und 25 A bestätigten die Zuordnung zu maximalem Strom am Netzanschluss und maximalem Strom der Versorgungsleitung. <code>lot.dyn</code> wird bewusst nicht veröffentlicht.</li>
    <li><code>configLoadBalancingPhaseAssignment</code><br>Fester dreistelliger Vektor aus <code>map</code>. Die bestätigten einphasigen Vektoren <code>[1,0,0]</code>, <code>[0,1,0]</code> und <code>[0,0,1]</code> werden als <code>L1</code>, <code>L2</code> und <code>L3</code> dargestellt. Jede Permutation von <code>1,2,3</code> wird akzeptiert und in der konfigurierten Reihenfolge ausgegeben, beispielsweise <code>[2,3,1]</code> als <code>L2 L3 L1</code>. Zweiphasige, fehlerhafte oder anderweitig unbestätigte Vektoren erhalten das vorherige Reading.</li>
    ...
    <li><code>deviceControllerFirmwareVersion</code>, <code>deviceControllerFirmwareCRC</code>, <code>deviceControllerFirmwareIntegrity</code>, <code>deviceControllerResetReason</code>, <code>deviceControllerMidFirmwareVersion</code>, <code>deviceControllerHardwareId</code><br>Rohe Strings aus den entsprechenden Feldern des beobachteten <code>cc4</code>-Objekts. Sie werden im gemeinsamen Intervall ohne Idle-Sperre veröffentlicht. Eine Beziehung zum Haupt-Firmware-Reading, Enums, Token-Dekodierung oder ein Health-Gesamturteil werden nicht abgeleitet.</li>
    <li><code>deviceControllerStackSize</code><br>Roher nicht negativer Ganzzahlwert aus <code>cc4.stack_size</code>, veröffentlicht mit den übrigen Gerätegesundheitswerten. Einheit und genaue Bedeutung bleiben unbestätigt.</li>
    ...
    <li><code>diag_pvopt_phaseWishMode</code><br>Optionaler interner Phasenwunsch aus <code>pwm</code>: <code>force3</code>, <code>wish1</code>, <code>wish3</code> oder <code>unknown:&lt;Rohwert&gt;</code>. Er ist von der konfigurierten Einstellung <code>configPhaseSwitchMode</code> getrennt; daraus werden weder Timer noch ein tatsächlicher Phasenwechsel abgeleitet.</li>
  ...
  <ul>
    ...
    <li><code>lastCommandRequestId</code>, <code>lastCommandStatus</code>, <code>lastCommandError</code><br>
        Korrelation und Ergebnis des letzten gesicherten Befehls. Statuswerte sind <code>pending</code>, <code>success</code>, <code>failed</code> oder <code>timeout</code>. Wird eine aktive Sitzung vor der Antwort invalidiert, werden alle ausstehenden Requests und ihr Timeout entfernt; der neueste Request endet als <code>failed</code> mit einem stabilen redigierten Grund wie <code>connection lost</code>, <code>device disabled</code>, <code>credentials changed</code>, <code>authentication aborted</code>, <code>lifecycle timeout</code>, <code>reconnect requested</code>, <code>definition changed</code> oder <code>session replaced</code>. Einzige Ausnahme ist ein Verbindungsverlust, während ein ausstehender Request den Reboot-Protokollschlüssel <code>rst</code> verwendet: Da das Gerät vor einer Antwort neu starten kann, wird dieser Request als <code>success</code>/<code>none</code> abgeschlossen; explizite Responses und der normale Timeout bleiben maßgeblich. Undefine und Shutdown unterdrücken neue Diagnose-Events.</li>
  </ul>
</ul>

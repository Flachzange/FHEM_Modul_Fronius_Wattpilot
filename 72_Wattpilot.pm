...
<ul>
  ...
  <ul>
    ...
    <li><code>set &lt;name&gt; pvBattery &lt;setting&gt; &lt;value&gt;</code><br>
        ...
        No reading is changed optimistically; only returned device status confirms a value. These setters are a Flex-43.4 verification candidate and must not be treated as device-side validated until controlled write, response, readback, and restore tests are recorded.</li>
  ...
  <ul>
    ...
    <li><code>configPvBatteryDischargeStartTime</code>, <code>configPvBatteryDischargeStopTime</code><br>App start/stop times from <code>pdls</code> and <code>pdlo</code>, converted from whole seconds after midnight to <code>HH:MM</code>. The six configuration mappings were matched to simultaneous Solar.wattpilot app values on one Flex Home 22 C6 running firmware 43.4. The grouped setters are implemented for controlled verification; broader firmware/model scope and actual device-side acceptance remain unverified until the live test is recorded.</li>
...
<ul>
  ...
  <ul>
    ...
    <li><code>set &lt;name&gt; pvBattery &lt;Einstellung&gt; &lt;Wert&gt;</code><br>
        ...
        Kein Reading wird optimistisch geändert; nur vom Gerät zurückgelieferter Status bestätigt einen Wert. Diese Setter sind ein Verifikationskandidat für Flex 43.4 und dürfen erst nach dokumentierten Schreib-, Response-, Readback- und Restore-Tests als geräteseitig bestätigt gelten.</li>
    <li><code>set &lt;name&gt; reconnect</code><br>
  ...
  <ul>
    ...
    <li><code>configPvBatteryDischargeStartTime</code>, <code>configPvBatteryDischargeStopTime</code><br>App-Start-/Stoppzeiten aus <code>pdls</code> und <code>pdlo</code>, von ganzen Sekunden seit Mitternacht nach <code>HH:MM</code> umgerechnet. Die sechs Konfigurationszuordnungen wurden auf einem Flex Home 22 C6 mit Firmware 43.4 anhand zeitgleich übereinstimmender Solar.wattpilot-App-Werte belegt. Die gruppierten Setter sind für einen kontrollierten Verifikationstest implementiert; weitere Firmware-/Modellstände und die tatsächliche geräteseitige Annahme bleiben bis zum Realtest unbestätigt.</li>

...
Version 2.1.9 adds `t/reboot_command.t` and expands the declarative command inventory to fifteen public Set commands. The focused suite covers the exact secured `rst` trigger payload, response-first and disconnect-first completion, both audited DevIo reconnect-ownership paths, bounded timeout behavior, disabled/disconnected/unauthenticated rejection, and preservation of the established `connection lost` failure for all non-reboot commands. This deterministic harness does not establish that the physical device accepts boolean `true`; a real Wattpilot test remains required.

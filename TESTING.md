...
`t/pv_battery_settings.t` protects the version-2.0.9 grouped setter implementation for issue #52 and the version-2.1.13 combined discharge command from issue #107. It loads a minimal sanitized fixture derived from one simultaneous Flex 43.4 app/status observation, verifies the six exact `configPvBattery...` names, checks fullStatus and deltaStatus updates, preserves existing readings for missing, null, type-invalid, out-of-range, and non-minute clock values, and covers `00:00` and `24:00` read boundaries. It proves that the grouped `pvBattery` command remains available, validates all six subcommands, exact protocol keys and JSON types, strict percentage and clock syntax, no write for invalid input, no optimistic reading update, successful status confirmation, and unchanged confirmed readings after a failed response. For `pvBatteryDischarge` it additionally covers the exact two-widget FHEMWEB schema, whitespace and comma input, both safe write orders, mandatory paired values, overlap rejection, first- and second-step rejection, first- and second-step timeout, connection abort, local second-step send failure, confirmed partial application, and terminal command diagnostics.
...
`t/pv_battery_diagnostics.t` covers the version-2.1.7 raw diagnostic publication of `fbuf_akkuMode`, `fbuf_akkuSOC`, and `fbuf_pAkku`. It verifies fullStatus, deltaStatus, and matched-response handling; two-decimal numeric formatting plus unchanged string and `0|1` boolean publication; rejection of `null`, objects, and arrays; the common diagnostic idle gate and interval owner; immediate cleanup when diagnostics are disabled; the deliberate absence of the former standalone SOC/power readings; and deliberate absence of the former normal `pvBatteryModeCode` reading. It also proves that the diagnostic owner cannot republish cached electrical values and that `interval=0` disables rate limiting for eligible diagnostics.
...
`t/reboot_command.t` covers issue #93 as a device command distinct from local `reconnect`. It verifies `reboot:noArg` discovery and exact arity, the canonical `securedMsg` wrapper around `setValue` with key `rst` and JSON boolean `true`, ordinary success and rejection responses, expected ordinary-EOF and WebSocket-Close completion, unchanged DevIo reconnect ownership, timeout cleanup when neither response nor disconnect arrives, and negative controls proving that unrelated commands still fail on connection loss. The transport-loss exception is request-local metadata and does not weaken general pending-command finalization.
...
`t/runtime_fixes_2_0_3.t` protects the version-2.0.3 corrections. It proves that `eto` and `wh` update while an idle `nrg` group is suppressed, that enabling `update_while_idle` still admits the real measurement array, that Define and reload-style Initialize expose the module version without lifecycle side effects, that a device `hello.version` updates only `deviceFirmwareVersion`, that `hello.protocol` remains distinct from `status.proto`, that the empirically observed startup types `clearInverters`, `updateInverter`, and `clearSmips` remain level-4-visible but produce no level-3 unsupported warning, and that all other unsupported message types produce useful but bounded/redacted diagnostics without payload logging.
...
`t/lifecycle_stateformat.t` covers issue #102 against FHEM mirror revision `0ae38bf79d19d8d598c065bf84b3990b33063c4b`. The double models `readingsEndUpdate` ordering from the inspected core: the public reading is stored, `evalStateFormat` rewrites Internal `STATE`, and only then is the event delivered. Tests cover complete and partial `fullStatus`, `deltaStatus`, timeout cancellation, watchdog operation with compound display text, manual reconnect, restored and directly manipulated public readings, reentrant Set execution, conservative process recreation, stale lifecycle callbacks, and reload invalidation with one replacement reconnect.
...
Version 2.1.8 adds `t/device_health_diagnostics.t`. It covers all seven observed `cc4` members as ordinary interval-controlled `device_health` readings and all six `tma` positions as optional numeric diagnostic readings. The test verifies partial nested updates, two-decimal temperature formatting, idle publication for controller fields, preservation across missing/`null`/wrong-type values and malformed containers, immediate diagnostic cleanup, and negative controls for unimplemented maximum-temperature, derating, and combined-health readings. The declarative-schema regression now constructs root, fixed-array-position, and one-level-object-member status inputs and audits 68 consumed status paths and 86 public readings.

Version 2.1.9 adds `t/reboot_command.t` and expands the declarative command inventory to fifteen public Set commands. The focused suite covers the exact secured `rst` trigger payload, immediate `state=rebooting`, response-first and disconnect-first completion, both audited DevIo reconnect-ownership paths, restoration to `connected` after rejection, malformed response, or timeout on a live authenticated session, rejection of additional secured commands during the reboot transition, disabled/disconnected/unauthenticated rejection, and preservation of the established `connection lost` failure for all non-reboot commands. This deterministic harness does not establish that the physical device accepts boolean `true`; a real Wattpilot test remains required.

Version 2.1.10 adds `t/inbound_watchdog.t` for issue #90. The regression reproduces a connected, locally open `wattpilot_flex` WebSocket session that receives no complete JSON documents, verifies the independent 30-second watchdog check and at-least-180-second inactivity boundary, and proves exactly one lifecycle-safe reconnect. Complete, incomplete, and invalid JSON, fresh activity immediately before the boundary, obsolete lifecycle callbacks, repeated initialization, manual reconnect, ordinary socket closure, disable/undefine/shutdown cleanup, the `rebooting` transition, independence from `interval` and `update_while_idle`, and the negative legacy `devicetype=wattpilot` control are covered. The persistent `connectionLastReconnectReason` and `connectionAutomaticReconnectCount` readings are checked for manual, socket, lifecycle, idle-refresh, repeated, and inbound-timeout paths. The real-device trigger was a Wattpilot Flex Home 22 C6 running firmware 43.4 that rebooted while FHEM retained an apparently open old session; reset `uptime` and increased `deviceRebootCount` confirmed the device reboot, while a manual local reconnect restored input. The watchdog fixes the stale local session and does not claim to diagnose or prevent the device-side reboot. A real-device automatic-watchdog recovery test remains required before merge.
...
Version 2.1.13 extends `t/pv_battery_settings.t`, the declarative command schema, the architecture characterization, the public-interface guard, and reload/version assertions for issue #107. The complete local suite passes with 33 test files and 3,652 tests, followed by strict META, command-reference, and repository checks. The isolated container used temporary external compatibility modules outside the repository for `JSON`, `Crypt::PBKDF2`, and `Crypt::URandom`; the fixed PBKDF2 vector passed and the optional bcrypt vector remained skipped because `Crypt::Bcrypt` was unavailable. The new coverage verifies the exact two-control `widgetList`, whitespace/comma parsing, mandatory paired inputs, safe enable/disable ordering, response-gated second dispatch, overlap protection, reentrant cleanup suppression, no optimistic readings, and step-specific rejection, timeout, abort, local send, and partial-failure diagnostics. No real FHEMWEB, WebSocket, or physical-device test of the combined sequence was performed in this implementation environment.

...
Version 2.1.7 adds `t/identity_diagnostic_readings.t`. It covers source-separated identity publication, immediate-on-change reconnect behavior, raw interval-controlled `rbc`/`rbt`, the independent uptime idle gate, all twelve optional raw diagnostics, JSON number/string/boolean handling, rejection of `null`/objects/arrays, attribute validation, immediate reading/cache cleanup on disable or attribute deletion, and reload cleanup when diagnostics are effectively off. The test suite does not assign protocol meanings or units to the raw research fields.

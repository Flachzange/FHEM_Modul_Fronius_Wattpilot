...
Version 2.0.8 adds six read-only stationary-PV-battery configuration readings: `configPvBatteryChargeAboveStateOfCharge` (`fam`), `configPvBatteryDischargeEnabled` (`pdte`), `configPvBatteryDischargeUntilStateOfCharge` (`pdt`), `configPvBatteryDischargeTimeLimitEnabled` (`pdle`), `configPvBatteryDischargeStartTime` (`pdls`), and `configPvBatteryDischargeEndTime` (`pdlo`). The mapping is evidenced by exact agreement between a simultaneous Solar.wattpilot app view and a Flex Home 22 C6 firmware-43.4 `fullStatus`. No battery Set command is exposed until controlled write, response, readback, and restore testing is complete.

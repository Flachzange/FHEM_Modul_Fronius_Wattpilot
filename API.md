...
Version 2.1.4 changes only the public Set command grouping for six already verified fields. `phaseSwitch mode|delay|minInterval` writes `psm`, `mpwst`, and `mptwt`; `minimumCharging duration|interval|pauseDuration` writes `fmt`, `mci`, and `mcpd`. The former individual Set names are removed without aliases. Protocol keys, JSON types, units, validation, secured request handling, device-confirmed readings, and rate limits are unchanged.

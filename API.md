...
Version 2.0.10 changes only Set-list metadata and discovery behavior: `reconnect` is advertised as `reconnect:noArg`, and `set <name> ?` returns the same authoritative command list even when the device attribute `disable=1` is active. Runtime commands remain rejected while disabled.

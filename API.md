...
Version 2.1.7 adds exact status-level identity readings (`deviceType`, `deviceModel`, `deviceSubType`, and `deviceVariant`), separates `helloProtocol` from `statusProtocol`, and exposes raw `rbc`/`rbt` values as interval-controlled `deviceRebootCount`/`deviceUptime`. It also adds the boolean `diagnosticReadings` attribute and twelve optional raw `diag_...` readings for field research. The optional values retain their protocol wording, are neither scaled nor rounded, and deliberately make no meaning, unit, sign, aggregation, or enum claim. Disabling or deleting the attribute removes those readings and clears their cache/dirty state.

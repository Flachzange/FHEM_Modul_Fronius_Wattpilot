...
The same release tightens local lifecycle and publication semantics without adding Wattpilot protocol fields: ordinary EOF remains DevIo-ReadyFn-owned while an ownerless WebSocket Close receives exactly one module reconnect; the first valid authenticated `fullStatus` or `deltaStatus`, including a partial full status, completes initialization; session invalidation finalizes pending secured commands with stable redacted causes; the one-shot Charging-to-Idle `nrg` refresh applies for both `update_while_idle` values; and changing or deleting `interval` to effective zero immediately flushes all currently eligible dirty telemetry owners in one transaction.

Version 2.1.6 corrects the remaining local interval-transition case: when `interval` changes between positive values and any telemetry owner is dirty, the old shared timer is replaced immediately with exactly one timer using the new interval. Queued values are not published early, stale callbacks cannot affect the replacement clock, Idle-gated owners remain passive, and no replacement clock is created when nothing is dirty.

Version 2.1.7 exposes the hello firmware string as `deviceFirmwareVersion`, adds exact status-level identity readings (`deviceType`, `deviceModel`, `deviceSubType`, and `deviceVariant`), separates `deviceHelloProtocol` from `deviceStatusProtocol`, exposes `rbc` as interval-controlled `deviceRebootCount`, and renders the live-observed millisecond value from `rbt` as interval-controlled cumulative `uptime` in `H:MM` after division by 1,000. It also adds the boolean `diagnosticReadings` attribute and fifteen optional raw `diag_...` readings for field research, including `diag_fbuf_akkuMode`, `diag_fbuf_akkuSOC`, and `diag_fbuf_pAkku` in place of the former standalone stationary-battery mode/SOC/power readings. The optional values retain their protocol wording. JSON numbers are rounded to exactly two decimal places without scaling, strings remain unchanged, and booleans become `0|1`; the interface deliberately makes no meaning, unit, sign, aggregation, or enum claim. Disabling or deleting the attribute removes those readings and clears their cache/dirty state. No aliases or automatic cleanup are provided for the removed development-stage names.

Version 2.1.8 adds six optional numeric readings `diag_temperatureSensor1` through `diag_temperatureSensor6` for the observed `tma[0]` through `tma[5]` positions and seven interval-controlled device-health readings for the observed `cc4` members: controller firmware version, CRC, firmware integrity, stack size, reset reason, MID firmware version, and hardware ID. Missing, `null`, structurally invalid, or wrong-type nested values preserve existing readings. The temperature positions are intentionally not assigned physical sensor locations, `cc4.reset_reason` is not equated with `deviceRebootCount`, and no maximum temperature, thermal-derating flag, aggregate health state, network diagnostics, OCPP diagnostics, or PV-link diagnostics are introduced.

Version 2.1.9 adds the no-argument `reboot` Set command. It sends protocol key `rst` with JSON boolean `true` through the existing authenticated `setValue`/`securedMsg` path. A regular response retains the normal success/rejection semantics. If the Wattpilot closes the transport before responding, only a pending request whose existing protocol key is `rst` is completed as successful; unrelated commands still fail with `connection lost`, and the established DevIo reconnect ownership remains unchanged. The pinned field registry describes `rst` as write-only `rebootCharger` with JSON type `any`; boolean `true` is the module's trigger representation pending real-device confirmation.

Public power values use watts and public durations use seconds while the corresponding protocol fields use milliseconds. These mappings combine official Fronius behavior documentation, pinned go-e API metadata, pinned Wattpilot-specific implementation evidence, and the sanitized Flex 43.4 field observation; they are not presented as an official Fronius WebSocket API specification. `set <name> reconnect` is a local lifecycle command that sends no protocol frame and must not be interpreted as a `fullStatus` request. `set <name> reboot` is a separate device command that writes `rst` and may intentionally cause the WebSocket to close before a response is received.

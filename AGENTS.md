...
- Treat Perl context as part of an API contract. Context-sensitive functions such as `gettimeofday()` must be forced into the intended scalar or list context when embedded in hashes, arrays, argument lists, callbacks, or expressions; test doubles must reproduce relevant `wantarray` behavior rather than returning one value unconditionally.
...
- Related stationary-PV-battery writes use the grouped top-level Set command `pvBattery` with explicit subcommands rather than separate top-level Set commands. The single deliberate exception is `pvBatteryDischarge`: enabling with `on|1` requires a `0-100` SoC threshold and uses the confirmed `pdt`-before-`pdte=true` sequence; disabling with `off|0` may omit the threshold and then writes only `pdte=false`, while an explicitly supplied threshold retains the confirmed `pdte=false`-before-`pdt` sequence. FHEMWEB keeps the established `off|on` dropdown and compact `SoC%` text field, and readings remain device-confirmed.

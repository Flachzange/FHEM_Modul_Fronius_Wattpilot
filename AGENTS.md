...
- Phase-switch mode/timing writes use `phaseSwitch mode|delay|minInterval`; minimum-charging timing writes use `minimumCharging duration|interval|pauseDuration`. Do not reintroduce the six former individual top-level Set commands or compatibility aliases.

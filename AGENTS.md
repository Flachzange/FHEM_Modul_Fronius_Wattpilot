...
- Related stationary-PV-battery writes use the grouped top-level Set command `pvBattery` with explicit subcommands rather than separate top-level Set commands. The single deliberate exception is `pvBatteryDischarge <0|1> <0-100>`, which requires both coupled discharge values, presents them as one two-widget FHEMWEB operation with an `off|on` dropdown and a compact `SoC%` text field, and sequences the existing `pdt`/`pdte` writes only after device confirmation.

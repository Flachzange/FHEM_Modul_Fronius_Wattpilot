...
Version 2.1.18 retains the authoritative publication policy for every public
...
Public measured and calculated physical values use `decimal2`, retaining trailing zeroes and normalizing rounded negative zero to positive zero. Explicit exceptions remain visible in the same inventory: booleans, integer codes and intentionally integral settings use `boolean` or `integer`; percentages use `percentage`; clocks and durations use `clock`, `seconds`, or `hours_minutes_ms`; text and enums retain their documented form. `uptime` converts a non-negative `rbt` millisecond value into cumulative `H:MM`, discarding remaining seconds and milliseconds. The fifteen raw-scalar `diag_...` readings use `diagnostic2`: validated JSON numbers are formatted with exactly two decimal places and rounded negative zero is normalized, strings remain unchanged, and JSON booleans become `0|1`. The two generic `diag_temperatureSensor1` and `diag_temperatureSensor2` readings and the four component-specific `diag_temperatureGridConnector`, `diag_temperatureCurrentSensor`, `diag_temperatureType2ScrewTerminals`, and `diag_temperatureMID` readings accept finite numeric array entries only and use `decimal2`. The component mapping is limited to the verified Wattpilot Flex Home 22 C6 observation; no scaling, unit, limit, derating threshold, or cross-model equivalence is applied. Validation always precedes formatting.
...
| `load_balancing_priority` | `configLoadBalancingPriority` | `configuration` | `status:lop` | `immediate` | `none` | `lop` | `enum` | `preserve` | `40=high`, `50=medium`, `60=low`; other non-negative integers become `unknown:<value>`. |
...
| `load_balancing_grid_connection_current` | `configLoadBalancingGridConnectionCurrent` | `configuration` | `status:lot.amp` | `immediate` | `none` | `lot_amp` | `integer` | `preserve` | Maximum grid-connection current; independent app change to 5 A confirmed `lot.amp`. |
| `load_balancing_supply_line_current` | `configLoadBalancingSupplyLineCurrent` | `configuration` | `status:lot.sta` | `immediate` | `none` | `lot_sta` | `integer` | `preserve` | Maximum supply-line current; independent app change to 25 A confirmed `lot.sta`; `lot.dyn` remains unexposed. |
| `load_balancing_phase_assignment` | `configLoadBalancingPhaseAssignment` | `configuration` | `status:map` | `immediate` | `none` | `map` | `phase_assignment` | `preserve` | Confirmed one-phase vectors render as `L1`, `L2`, or `L3`; every permutation of `1,2,3` is accepted and preserves configured order, including observed `[2,3,1]` → `L2 L3 L1`; two-phase and malformed vectors preserve the previous reading. |
...
| `device_controller_firmware_version` | `deviceControllerFirmwareVersion` | `device_health` | `status:cc4.firmware_version` | `interval` | `none` | `device_health` | `text` | `preserve` | Raw controller firmware-version string from the observed `cc4` object; relationship to the main firmware reading remains unconfirmed. |
| `device_controller_firmware_crc` | `deviceControllerFirmwareCRC` | `device_health` | `status:cc4.firmware_crc` | `interval` | `none` | `device_health` | `text` | `preserve` | Raw controller firmware-CRC string from the observed `cc4` object; no decoding is applied. |
| `device_controller_firmware_integrity` | `deviceControllerFirmwareIntegrity` | `device_health` | `status:cc4.firmware_integrity` | `interval` | `none` | `device_health` | `text` | `preserve` | Raw controller integrity-status string; no enum or health verdict is inferred. |
| `device_controller_stack_size` | `deviceControllerStackSize` | `device_health` | `status:cc4.stack_size` | `interval` | `none` | `device_health` | `integer` | `preserve` | Raw non-negative controller stack-size value; exact meaning and unit remain unconfirmed. |
| `device_controller_reset_reason` | `deviceControllerResetReason` | `device_health` | `status:cc4.reset_reason` | `interval` | `none` | `device_health` | `text` | `preserve` | Raw controller reset-reason string; tokens are not decoded and it is not equated with `deviceRebootCount`. |
| `device_controller_mid_firmware_version` | `deviceControllerMidFirmwareVersion` | `device_health` | `status:cc4.mid_firmware_version` | `interval` | `none` | `device_health` | `text` | `preserve` | Raw MID firmware-version string from the observed controller object. |
| `device_controller_hardware_id` | `deviceControllerHardwareId` | `device_health` | `status:cc4.hwid` | `interval` | `none` | `device_health` | `text` | `preserve` | Raw controller hardware identifier from the observed `cc4` object. |
| `diag_temperature_sensor_1` | `diag_temperatureSensor1` | `optional_diagnostic` | `status:tma[0]` | `interval` | `diagnostic` | `diagnostic` | `decimal2` | `preserve` | Optional numeric temperature-array position 1; physical sensor location and unit are not claimed by this module. |
| `diag_temperature_sensor_2` | `diag_temperatureSensor2` | `optional_diagnostic` | `status:tma[1]` | `interval` | `diagnostic` | `diagnostic` | `decimal2` | `preserve` | Optional numeric temperature-array position 2; physical sensor location and unit are not claimed by this module. |
| `diag_temperature_sensor_3` | `diag_temperatureGridConnector` | `optional_diagnostic` | `status:tma[2]` | `interval` | `diagnostic` | `diagnostic` | `decimal2` | `preserve` | Optional numeric `tma[2]` value mapped to the grid connector on the verified Wattpilot Flex Home 22 C6; unit and cross-model equivalence remain unconfirmed. |
| `diag_temperature_sensor_4` | `diag_temperatureCurrentSensor` | `optional_diagnostic` | `status:tma[3]` | `interval` | `diagnostic` | `diagnostic` | `decimal2` | `preserve` | Optional numeric `tma[3]` value mapped to the current sensor on the verified Wattpilot Flex Home 22 C6; unit and cross-model equivalence remain unconfirmed. |
| `diag_temperature_sensor_5` | `diag_temperatureType2ScrewTerminals` | `optional_diagnostic` | `status:tma[4]` | `interval` | `diagnostic` | `diagnostic` | `decimal2` | `preserve` | Optional numeric `tma[4]` value mapped to the Type 2 screw terminals on the verified Wattpilot Flex Home 22 C6; unit and cross-model equivalence remain unconfirmed. |
| `diag_temperature_sensor_6` | `diag_temperatureMID` | `optional_diagnostic` | `status:tma[5]` | `interval` | `diagnostic` | `diagnostic` | `decimal2` | `preserve` | Optional numeric `tma[5]` value mapped to the MID on the verified Wattpilot Flex Home 22 C6; unit and cross-model equivalence remain unconfirmed. |

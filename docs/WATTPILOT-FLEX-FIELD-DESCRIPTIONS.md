...
| `fbuf_akkuMode` | `pvBatteryModeCode` / historical `akkuMode` | Version 2.0.6 exposes the observed non-negative integer unchanged. No text enum is claimed. Normal delta updates follow the module-side `interval` gate and the shared `update_while_idle` policy; complete initial status and matched responses bypass only the interval gate. | R compatibility implementation; no setter | sanitized Flex 43.4 field/type/value plus pinned Wattpilot-specific alias; rate limiting is module policy, not protocol semantics |
| `fbuf_akkuSOC` | `pvBatteryStateOfCharge` / historical `akkuSoc` | Version 2.0.6 exposes the stationary PV-battery state of charge as a finite value from 0 through 100 percent. Normal delta updates follow the module-side `interval` gate and the shared `update_while_idle` policy; complete initial status and matched responses bypass only the interval gate. | R compatibility implementation; no setter | sanitized Flex 43.4 field/type/value plus pinned Wattpilot-specific meaning; one observation does not establish broader firmware scope |
| `fbuf_pAkku` | `pvBatteryPower` / historical `powerAkku` | Version 2.0.6 exposes the signed finite value in W, formatted to two decimal places. A historical source suggests a sign convention, but controlled Flex confirmation is still missing, so no charge/discharge direction is assigned. Normal delta updates follow the module-side `interval` gate and the shared `update_while_idle` policy; complete initial status and matched responses bypass only the interval gate. | R compatibility implementation; no setter | sanitized Flex 43.4 field/type/value plus pinned Wattpilot-specific power/W candidate; sign semantics remain unconfirmed |

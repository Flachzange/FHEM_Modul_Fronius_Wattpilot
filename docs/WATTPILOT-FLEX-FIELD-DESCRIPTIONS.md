...
|---|---|---|---|---|
| `fam` | `configPvBatteryChargeAboveSoC`; grouped Set `pvBattery chargeAboveSoC`; app `Charge above` | The reader accepts finite 0–100 values; the setter candidate sends whole 0–100 integers. The exact app/status pair was 60/60 on one Flex 43.4. | R/W implementation candidate; live write/readback/restore unverified | simultaneous Solar.wattpilot app and `fullStatus` evidence; historical `pvBatteryLimit` alias is secondary |
...
| `fbuf_akkuSOC` | `pvBatterySoC` / historical `akkuSoc` | Version 2.0.6 exposes the stationary PV-battery state of charge as a finite value from 0 through 100 percent with exactly one decimal place. All message types follow the same shared module-side `interval` gate and `update_while_idle` policy. | R compatibility implementation; no setter | sanitized Flex 43.4 field/type/value plus pinned Wattpilot-specific meaning; one observation does not establish broader firmware scope |
...
| `pdte` | `configPvBatteryDischargeEnabled`; grouped Set `pvBattery dischargeEnabled`; app `Discharge until` | The reader exposes the observed boolean as `0`/`1`; the setter candidate sends a JSON boolean. | R/W implementation candidate; live write/readback/restore unverified | simultaneous app `off` and status `false` on one Flex 43.4 |
| `pdt` | `configPvBatteryDischargeUntilSoC`; grouped Set `pvBattery dischargeUntilSoC`; app `State of charge SoC` | The reader accepts finite 0–100 values; the setter candidate sends whole 0–100 integers. | R/W implementation candidate; live write/readback/restore unverified | simultaneous app/status value `57` on one Flex 43.4 |
| `pdle` | `configPvBatteryDischargeTimeLimitEnabled`; grouped Set `pvBattery dischargeTimeLimitEnabled`; app `Limit discharging time` | The reader exposes the observed boolean as `0`/`1`; the setter candidate sends a JSON boolean. | R/W implementation candidate; live write/readback/restore unverified | simultaneous app `on` and status `true` on one Flex 43.4 |
| `pdls` | `configPvBatteryDischargeStartTime`; grouped Set `pvBattery dischargeStartTime`; app `Start` | Whole seconds after midnight rendered as `HH:MM`; setter candidate converts strict `00:00`–`23:59` to seconds. Observed `25200` = `07:00`. | R/W implementation candidate; live write/readback/restore unverified | simultaneous app/status evidence on one Flex 43.4 |
| `pdlo` | `configPvBatteryDischargeStopTime`; grouped Set `pvBattery dischargeStopTime`; app `End` | Whole seconds after midnight rendered as `HH:MM`; setter candidate converts strict `00:00`–`23:59` and the boundary `24:00` to seconds. Observed `72000` = `20:00`. | R/W implementation candidate; live write/readback/restore unverified | simultaneous app/status evidence on one Flex 43.4 |

...
| Current implementation behavior | Directly visible in root `72_Wattpilot.pm`; describes the version 2.0.5 runtime, not what the device specification promises. |
...
| Public FHEM interface contract | Names and values implemented by version 2.0.5; this still does not prove device semantics. |
...
| `fap` | boolean | `true` | Version 2.0.5 exposes `chargingPauseAllowed` as `0`/`1` and writes a JSON boolean through secured `setValue`. | boolean | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents Allow charging pause; sanitized capture confirms field/type; pinned go-e and Wattpilot-specific sources supply key/R/W association. Not an official Fronius WebSocket specification. |
...
| `fmt` | number | `300000` | Version 2.0.5 exposes `minimumChargeTime` in seconds and writes whole protocol milliseconds. | protocol ms; public s | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents Minimum charging time; sanitized capture confirms field/type/value; pinned sources supply key/R/W/ms association. |
...
| `frm` | number | `0` | Version 2.0.5 exposes `pvControlPreference`: `0 preferFromGrid`, `1 default`, `2 preferToGrid`, with `unknown:<value>` fallback; setter writes the matching integer. | enum | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents the three control behaviors; sanitized capture confirms field/type; pinned sources supply key/enum/R/W association. |
...
| `fup` | boolean | `true` | Version 2.0.5 exposes `pvSurplusEnabled` as `0`/`1` and writes a JSON boolean through secured `setValue`. | boolean | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents Use PV surplus; sanitized capture confirms field/type; pinned sources supply key/R/W association. |
...
| `fzf` | boolean | `false` | Version 2.0.5 exposes `zeroFeedInEnabled` as `0`/`1` and writes a JSON boolean through secured `setValue`. | boolean | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents Zero feed-in; sanitized capture confirms field/type; pinned sources supply key/R/W association. |
...
| `mptwt` | number | `600000` | Version 2.0.5 exposes `minimumPhaseSwitchInterval` in seconds and writes whole protocol milliseconds. | protocol ms; public s | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents Phase switch interval; sanitized capture confirms field/type/value; pinned sources supply key/R/W/ms association. |
...
| `psm` | number | `0` | Version 2.0.5 exposes `phaseSwitchMode`: `0 auto`, `1 force1`, `2 force3`, with `unknown:<value>` fallback; setter writes the matching integer. | enum | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents Automatic / Only 1-phase / Only 3-phase; sanitized capture confirms field/type; pinned sources supply key/enum/R/W association. |
...
| `spl3` | number | `5200` | Version 2.0.5 exposes `threePhaseSwitchPower` as a non-negative finite watt value and provides a secured setter. | W | R/W compatibility implementation; Flex write test pending | combined behavior/API/field evidence | Official Fronius Flex manual documents 3-phase power level; sanitized capture confirms field/type/value; pinned sources supply key/R/W/W-unit association. |

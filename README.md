...
Die Readings `pvBatteryStateOfCharge`, `pvBatteryPower` und `pvBatteryModeCode` beziehen sich ausschließlich auf den stationären PV-Speicher, nicht auf die Fahrzeugbatterie. Sie werden aus den vom Wattpilot gelieferten Feldern `fbuf_akkuSOC`, `fbuf_pAkku` und `fbuf_akkuMode` gelesen. Gültige Speicherwerte werden intern zwischengespeichert und gemeinsam mit dem nächsten zugelassenen `nrg`-Messzyklus veröffentlicht. Damit besitzen Spannung, Strom, Leistung und stationäre Speichertelemetrie genau eine gemeinsame `interval`-Zeitbasis und werden innerhalb derselben FHEM-Reading-Transaktion aktualisiert. Batterie-only-Nachrichten, vollständige Initialstatus-Nachrichten ohne gültiges `nrg` und zugeordnete Geräteantworten veröffentlichen die Speicherreadings nicht separat und verbrauchen das Intervall nicht. Für diese drei Werte gibt es in Version 2.0.6 bewusst keine Setter. Insbesondere werden weder eine unbestätigte Modus-Enum noch eine unbestätigte Vorzeichenbedeutung für die Speicherleistung erfunden. Schreibbare Speicherparameter wie `fam` bleiben bis zur eindeutigen Feld- und Schreibverifikation außerhalb der öffentlichen Schnittstelle.
...
* Nur ein zugelassener Messzyklus mit einem gültigen `nrg`-Array schreibt diese Zeitbasis fort. Gültige Batteriewerte aus zwischenzeitlichen Batterie-only-Nachrichten werden zwischengespeichert und im selben Reading-Zyklus mit dem nächsten `nrg` veröffentlicht.
* Konfigurationsnachrichten, Batterie-only-Nachrichten und ungültige beziehungsweise unvollständige `nrg`-Nachrichten verbrauchen das Intervall nicht. Ein vollständiger Initialstatus oder eine zugeordnete Geräteantwort ohne gültiges `nrg` umgeht den gemeinsamen Takt nicht.
...
Die operativen Status- und Konfigurations-Readings außerhalb der drei PV-Speicherreadings werden bei jeder gültigen Geräteinformation sofort verarbeitet und unterliegen weder `interval` noch `update_while_idle`. Die drei PV-Speicherreadings werden intern zwischengespeichert und ausschließlich zusammen mit einem zugelassenen `nrg`-Messzyklus veröffentlicht. Dadurch verwenden alle hochfrequenten Messreadings dieselbe `LAST_UPDATE`-Zeitbasis und dieselbe `update_while_idle`-Entscheidung. Fehlende, `null`- oder typfalsche Felder lassen bestehende Werte und den zuletzt gültigen Zwischenspeicher unverändert.

...
Das Passwort und der daraus abgeleitete Authentifizierungswert werden unter einem stabilen, auf der FUUID basierenden Schlüssel gespeichert. Vorhandene namensbasierte Schlüssel werden beim Laden oder Umbenennen erst nach erfolgreicher Speicherung des neuen Werts entfernt. Schlägt eine Rename-Migration fehl, werden frühere Namen getrennt für Passwort und Passwort-Hash als nicht-sensitive, FUUID-bezogene Pending-Metadaten persistent gespeichert. Damit bleiben Wiederholungsversuche auch über `rereadcfg`, Reload und Neustart mit einem neuen Device-Hash hinweg möglich. Nach erfolgreicher Migration oder Löschung werden diese Metadaten entfernt. `rereadcfg`, Reload, Disable und normales Undefine löschen keine Zugangsdaten; nur das tatsächliche Löschen des FHEM-Geräts entfernt sie.

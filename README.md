...
Das Passwort und der daraus abgeleitete Authentifizierungswert werden unter einem stabilen, auf der FUUID basierenden Schlüssel gespeichert. Vorhandene namensbasierte Schlüssel werden beim Laden oder Umbenennen erst nach erfolgreicher Speicherung des neuen Werts entfernt. Der Rename-Pfad berücksichtigt, dass FHEM den Namen bereits ändert und die Rückgabe von `RenameFn` verwirft. Frühere Namen werden deshalb getrennt für Passwort und Passwort-Hash als nicht-sensitive, FUUID-bezogene Pending-Metadaten persistent gespeichert; zusätzlich bindet ein persistenter Owner-Marker jede Legacy-Ressource an ihre FUUID. Ein Name allein berechtigt weder Migration noch Änderung oder Löschung. Fremde oder nicht verifizierbare Ressourcen bleiben erhalten und erzeugen eine ausdrückliche Credential-Fehlermeldung. Damit bleiben sichere Wiederholungsversuche auch über `rereadcfg`, Reload und Neustart mit einem neuen Device-Hash hinweg möglich. Nach erfolgreicher Migration oder Löschung werden die eigenen Metadaten entfernt. `rereadcfg`, Reload, Disable und normales Undefine löschen keine Zugangsdaten; nur das tatsächliche Löschen des FHEM-Geräts entfernt sie.
...
Credential-Lesezugriffe unterscheiden Wert vorhanden, Wert nicht vorhanden und Speicherfehler. Ein Speicher- oder Metadatenfehler wird in Define, Enable, Authentifizierung, gesicherten Befehlen und fehlgeschlagener Delete-Wiederherstellung als `credential error` behandelt und nicht als fehlendes Passwort ausgegeben.

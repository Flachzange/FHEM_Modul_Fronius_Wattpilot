...
Das Passwort und der daraus abgeleitete Authentifizierungswert werden ausschließlich unter stabilen FUUID-basierten Schlüsseln gespeichert. Ein Rename ändert diese Schlüssel nicht und führt keine Credential-Migration aus. `rereadcfg`, Reload, Disable, Rename und normales Undefine erhalten die Zugangsdaten; nur das tatsächliche Löschen des FHEM-Geräts entfernt die beiden eigenen stabilen Werte.

Beim Ändern des Passworts und beim Löschen liest das Modul zunächst Snapshots des stabilen Passworts und Passwort-Hashes. Schlägt eine Änderung teilweise fehl, werden bereits geänderte Werte in umgekehrter Reihenfolge wiederhergestellt; auch ein unvollständiger Rollback wird ausdrücklich gemeldet. Nach dem realen FHEM-Ablauf `UndefFn` gefolgt von einer fehlgeschlagenen `DeleteFn` werden `defptr`, ein ehrlicher Status und – nur bei aktivem Gerät mit vorhandenem Passwort – genau ein Reconnect-Timer wiederhergestellt.

Credential-Lesezugriffe unterscheiden Wert vorhanden, Wert nicht vorhanden und Speicherfehler. Ein Speicherfehler wird in Define, Enable, Authentifizierung, gesicherten Befehlen und fehlgeschlagener Delete-Wiederherstellung als `credential error` behandelt und niemals als fehlendes Passwort ausgegeben. Die veröffentlichten 1.6.x-Versionen sind die letzten Releases mit Unterstützung für namensbasierte Credential-Migration. Version 2.0 erfordert eine frische Definition und ein erneutes `set <name> Password <secret>`; alte namensbasierte Schlüssel werden weder gelesen noch automatisch bereinigt.

...
* Energy, electrical `nrg` telemetry, and stationary-battery telemetry have independent interval histories. No group can block another group or refresh its reading timestamps with stale cached values.
* Inside the interval, each group retains only its latest valid state. Publication requires a later valid input from that same group. Missing, `null`, wrong-type, or incomplete values consume no interval.
...
* `1`: real incoming idle values are processed according to each group's own `interval` history. After `car=2` changes to a valid non-charging state, one real `nrg` in the same message or within 30 seconds may bypass the electrical rate limit once so device-supplied zero values can correct stale readings.
...
* Energy readings remain interval-controlled while idle. Discrete status/diagnostic readings remain immediate-on-change. `pvBatteryModeCode` is such a discrete status value and is not part of the battery rate limit.

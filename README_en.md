...
The load-balancing readings expose only the core confirmed simultaneously between the Flex 43.4 status and app. Observed `clearInverters` and `updateInverter` messages maintain the candidate list, while `cci` identifies the selected source. The still ambiguous fields `loa`, `lom`, `los`, `lot`, `loty`, and `lopr` are not promoted to public semantics. A `loadBalancing` setter is deferred until writability, ranges, and dependencies have been reproducibly verified on the device.
...
A combined command is additionally available for the two related discharge settings. Enabling requires the SoC threshold; disabling may omit it:
...
`off` or `0` without a SoC writes only `dischargeEnabled` (`pdte=false`) and leaves the existing threshold unchanged. When a SoC is supplied while disabling, the safe two-step order remains: `pdte=false` first, then the new `pdt` value. Enabling writes `dischargeUntilSoC` (`pdt`) first and sends `pdte=true` only after device confirmation. FHEMWEB keeps the `off|on` dropdown and compact `SoC%` field; an empty field is valid only for `off`. Further writes to `pdt` or `pdte` are blocked while a related request is pending.
...
* All 29 `config...` readings remain immediate after valid device confirmation. Identity readings, `carState`, `chargingAllowed`, `temperatureCurrentLimit`, the four charging-decision readings, `loadBalancingSourceConnected`, and `errorCode` publish immediately, but only when their public value changes.
...
Controls the twenty-two optional diagnostic readings whose names begin with `diag_`: fifteen raw scalar fields, the mapped `diag_pvopt_phaseWishMode` enum, and six numeric positions from the observed `tma` array.
...
* `1`: valid values from the twenty-two selected protocol positions are published through the normal `interval` mechanism. They are eligible while the vehicle is charging or when `update_while_idle=1`.
* The protocol wording is retained exactly after the `diag_` prefix. For the fifteen raw scalar fields, JSON numbers are rounded to exactly two decimal places without scaling or conversion; strings remain unchanged and JSON booleans become `0` or `1`. `diag_pvopt_phaseWishMode` maps only integer `0|1|2` to `force3|wish1|wish3`; other integers remain explicit as `unknown:<value>`. No additional state, timer, or actual phase-switch decision is inferred. Missing fields, `null`, objects, arrays, and invalid values preserve the previous reading.
...
| `diag_pvopt_phaseWishMode` | Optional internal phase wish from `pwm`: `force3`, `wish1`, `wish3`, or `unknown:<raw-value>`. It is distinct from the configured phase-switch mode and does not by itself establish a timer or actual phase transition. |
...
| `configPvBatteryDischargeEnabled` | App switch “Discharge until” from `pdte`, exposed as `0` or `1`; writable through `set <name> pvBattery dischargeEnabled <0\|1>`. `set <name> pvBatteryDischarge off` or `0` disables without changing the SoC; `on` or `1` requires a threshold from `0` through `100`. |
| `configPvBatteryDischargeUntilSoC` | Associated app setting “State of charge SoC” from `pdt`, accepted as a percentage from `0` through `100`; writable through `set <name> pvBattery dischargeUntilSoC <0-100>`. The threshold is mandatory for `pvBatteryDischarge on\|1` and optional for `off\|0`. |

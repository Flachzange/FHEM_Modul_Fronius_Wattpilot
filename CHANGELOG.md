...
- Spannung, Strom, Leistung und die drei stationären Speicherreadings verwenden nun genau eine gemeinsame `interval`-Zeitbasis. Gültige Batterieinformationen werden zwischengespeichert und ausschließlich zusammen mit dem nächsten zugelassenen `nrg`-Messzyklus in derselben FHEM-Reading-Transaktion veröffentlicht; eine separate `LAST_BATTERY_UPDATE`-Historie gibt es nicht.
- Regression im Messwert-Rate-Limit behoben: Batterie-, Konfigurations- und ungültige oder unvollständige `nrg`-Deltas verbrauchen `LAST_UPDATE` nicht und können nachfolgende Spannungs-, Strom- und Leistungswerte nicht verdrängen. `update_while_idle` gilt einmalig für den gemeinsamen Messzyklus.

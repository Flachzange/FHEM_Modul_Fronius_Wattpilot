...
- Split cumulative energy, electrical `nrg`, and stationary-battery SOC/power into independent `energy`, `nrg`, and `battery` interval histories. A message can publish only valid current input from its own group, so battery, energy, and discrete status can no longer starve fresh electrical readings or renew stale cross-group timestamps.
- `energyTotal` and `energySincePlugIn` now obey `interval`; `interval=0` disables all three telemetry rate limits. Missing, `null`, malformed, wrong-type, out-of-range, or incomplete input preserves readings and consumes no interval.
...
- Historisch konnten gültige Batterieinformationen den gemeinsamen Messzyklus auslösen. Version 2.1.1 hebt dieses Verhalten ausdrücklich auf: Jede Telemetriegruppe veröffentlicht nur ihre eigenen neuesten gültigen Werte und besitzt eine unabhängige Intervallhistorie.
...
- Version 2.0.3 nahm `energyTotal` und `energySincePlugIn` historisch aus dem damaligen gemeinsamen `nrg`-/Idle-Gate heraus. Seit Version 2.1.1 bleiben sie zwar unabhängig vom Idle-Gate, unterliegen aber einer eigenen `energy`-Intervallhistorie.

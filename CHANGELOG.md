...
- Zusätzlich zu den unveränderten Rohwerten `chargingDecisionCode` und `chargingDecisionInternalCode` liefern `chargingDecision` und `chargingDecisionInternal` direkt nutzbare lowerCamelCase-Klartextwerte. Die Kompatibilitätszuordnung folgt der gepinnten go-e-Enum; unbekannte Codes bleiben als `unknown:<Code>` sichtbar. Der interne `msi`-Wert wird unabhängig mit derselben Tabelle abgebildet.
- Zu den unveränderten Rohwerten `chargingDecisionCode` (`modelStatus`) und `chargingDecisionInternalCode` (`msi`) kommen die Klartext-Readings `chargingDecision` und `chargingDecisionInternal`. Sie verwenden die gepinnte offizielle go-e-`modelStatus`-Enum mit stabilem Fallback `unknown:<Code>`; die Übertragbarkeit auf Wattpilot Flex ist empirisch plausibel, aber keine offizielle Fronius-Spezifikation.

...
- Replaces the `pvBatteryDischarge` FHEMWEB `0|1` selector with a dropdown containing only `off` and `on`; both values map to the existing JSON boolean behavior.
- Replaces the 101-entry SoC selector with a compact free-text field using the placeholder `SoC%`. The existing whole-number validation from `0` through `100` remains authoritative and invalid text sends no device command.
- Keeps the documented command-line syntax `set <name> pvBatteryDischarge <0|1> <0-100>` unchanged while accepting the comma-separated `off|on,<SoC>` representation generated by FHEMWEB.
- Leaves write ordering, response gating, overlap protection, device-confirmed readings, and partial-failure handling unchanged. No real FHEMWEB or physical-device test was performed in the implementation environment.

## [v2.1.13] - 2026-07-19

### Combined PV-battery discharge command

- Adds `set <name> pvBatteryDischarge <0|1> <0-100>` as a two-value shortcut for `dischargeEnabled` and `dischargeUntilSoC`; both values are mandatory so an old threshold cannot be reused accidentally.
- Exposes two FHEMWEB controls through `widgetList`: a `0|1` selector and a whole-number SoC selector from `0` through `100`. The parser accepts the ordinary whitespace form and the comma-separated representation generated by FHEMWEB.
- Executes the two secured writes sequentially. Enabling confirms `pdt` before sending `pdte=true`; disabling confirms `pdte=false` before sending `pdt`.
- Blocks overlapping writes to `pdt` and `pdte` while either a combined sequence or related grouped write is pending. Existing grouped `pvBattery` subcommands remain available.
- Preserves device-confirmed semantics without optimistic reading updates. Rejection, timeout, connection loss, or a local second-step send failure identifies the failed subcommand and reports partial application when the first step was already confirmed.
- Extends command-schema, interface, documentation, and PV-battery regression coverage. No physical-device or real-FHEM test of the new combined sequence was performed in the implementation environment.
...
- Publishes the lifecycle state `rebooting` immediately after the secured reboot request has been sent. A rejection, malformed response, or timeout on an otherwise live authenticated session restores `connected`; an actual socket close continues through the existing `disconnected`/reconnect lifecycle.
...
- Adds always-visible `deviceFirmwareVersion`, `deviceType`, `deviceModel`, `deviceSubType`, `deviceVariant`, `deviceHelloProtocol`, and `deviceStatusProtocol` readings. The development-stage name `firmwareVersion` is replaced without an alias; the two protocol values remain source-separated and exact device strings are not mapped to invented models.
...
- Adds boolean attribute `diagnosticReadings:0,1` with effective default `0`. Enabling it exposes fifteen exact `diag_`-prefixed raw scalar fields for real-device investigation, including `diag_fbuf_akkuMode`, `diag_fbuf_akkuSOC`, and `diag_fbuf_pAkku` in place of the former standalone stationary-battery mode/SOC/power readings; JSON numbers are rounded to exactly two decimal places without scaling, strings remain unchanged, and JSON booleans become `0|1`; no semantic interpretation is added.
